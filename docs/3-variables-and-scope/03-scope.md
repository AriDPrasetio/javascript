...
>
  <!-- buat wadah utama dashboard dengan garis tepi -->
  <p>Tema Global: <strong id="global-text">dark</strong></p>
  <!-- buat paragraf untuk menampilkan tema global dengan teks tebal -->
  <div id="inner-banner" style="padding: 8px; border-radius: 4px;">
    <!-- buat wadah bagian dalam untuk banner -->
    Banner Blok Scope: <span id="banner-text">-</span>
    <!-- tempat untuk menampilkan teks banner -->
  </div>
</div>
```
...
// buat wadah tetap appTheme dengan nilai "dark" di luar fungsi.

function renderDashboard() {
  // buat fungsi renderDashboard:
  const pageTitle = "Dashboard Analytics"; // 2. Function Scope (hanya hidup di dalam fungsi ini)
  // simpan teks judul ke dalam pageTitle yang cuma bisa dipakai di fungsi ini.
  const bannerEl = document.querySelector("#inner-banner");
  // cari elemen HTML dengan ID inner-banner, simpan ke bannerEl.

  if (true) {
    // jika kondisinya benar (selalu jalan di sini):
    // 3. Block Scope & Shadowing: Variabel 'appTheme' lokal menutupi variabel global di blok ini saja
    const appTheme = "emerald";
    // buat appTheme baru khusus di dalam blok ini (menutupi yang di luar).
    const badgeBg = "#10b981"; // Block Scope
    // buat warna latar belakang yang cuma hidup di dalam blok ini.

    bannerEl.style.backgroundColor = badgeBg;
    // ubah warna latar elemen banner.
    bannerEl.style.color = "#ffffff";
    // ubah warna teks banner menjadi putih.
    bannerEl.textContent = `${pageTitle} — Mode: ${appTheme} (Shadowed)`;
    // ganti teks pada banner dengan judul dan tema lokal.
  }
...
renderDashboard();
// jalankan fungsi renderDashboard.
document.querySelector("#global-text").textContent =
  `${appTheme} (Variabel global tidak terpengaruh)`;
// perbarui teks global dengan variabel appTheme yang dari luar.
```
